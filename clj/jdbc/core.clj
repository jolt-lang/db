...
(defn connection
  ...
  (let [{:keys [vendor] :as spec} (normalize-spec spec)]
    (case vendor
      ...
      "postgresql"
      ;; db.pg is statically required so AOT builds include the PostgreSQL
      ;; implementation. Its FFI bindings resolve libpq symbols lazily on first use.
...
(defn- pg-eval [conn sql params]
  (pg/exec (:handle conn) (pg-placeholders sql) params))
  ...
  ([conn q opts]
   (let [[sql params] (sqlvec q)
         rows (case (:vendor conn)
                ...
                :postgresql (pg/all (:handle conn) (pg-placeholders sql) params))]
